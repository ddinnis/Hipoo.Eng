...
public class EpisodeUpdatedEventHandler : INotificationHandler<EpisodeUpdatedEvent>
{
    ...
    public EpisodeUpdatedEventHandler(ICapPublisher capBus)
    {
        _capBus = capBus;
    }
    ...
    public Task Handle(EpisodeUpdatedEvent notification, CancellationToken cancellationToken)
    {
        ...
        if (episode.IsVisible)
        {
            ...
            _capBus.Publish("ListeningEpisode.Updated", new { Id = episode.Id, episode.Name, Sentences = sentences, episode.AlbumId, episode.Subtitle, episode.SubtitleType });
        }
        else
        {
            _capBus.Publish("ListeningEpisode.Hidden", new { Id = episode.Id });
        }
